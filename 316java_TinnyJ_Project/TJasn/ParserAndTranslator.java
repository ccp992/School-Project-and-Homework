...
public final class ParserAndTranslator {
  ...
  private static void whileStmt() throws SourceFileErrorException
  {
	...
	JUMPONFALSEinstr jumpFalse = new JUMPONFALSEinstr(Instruction.OPERAND_NOT_YET_KNOWN);
	
	statement();
	
	new JUMPinstr(n);
	jumpFalse.fixUpOperand(Instruction.getNextCodeAddress());
  ...
  private static void expr6() throws SourceFileErrorException
  {
	...
	while(getCurrentToken() == AND){
		Symbols symbol = getCurrentToken();
